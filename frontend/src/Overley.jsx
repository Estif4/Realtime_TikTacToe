...
export default function Overlay({
  ...
  useEffect(() => {
    socket.on("makegavefinal", (data) => {
      setBoard(data.boardreset);
      setCurrentPlayer(data.curentplayer);
      setGameOver(data.gamefinal);
      setWinner(data.win);
      setPosition(data.postion);
    });

    return () => {
      socket.off("makegavefinal");
    };
  }, []);

  function handler() {
    socket.on("makegavefinal", (data) => {
      socket.emit("makegavefinal", data);
    });

    let gameState = {
      boardreset: Array(9).fill(""),
      curentplayer: "X",
      gamefinal: false,
      win: null,
      postion: [],
    ...
    socket.emit("gavefinal", (gameState));
  }
  ...
  return (
    <div
      className={`overlay absolute inset-0 ${
        gameOver ? "" : "hidden"
      } flex flex-col justify-center items-center : bg-slate-900  p-12   z-10 opacity-90 space-y-4`}>
